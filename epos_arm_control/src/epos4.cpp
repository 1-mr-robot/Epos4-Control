...
void arm_config(const epos_arm_control::epos::ConstPtr& msg)
{
  arm_control = *msg;
  cout << "received" << endl;
  flag = 1;
...
int main(int argc, char** argv)
{
	...
	ros::init(argc,argv, "epos4_motor_command");
    ros::NodeHandle n;
	...
	ros::Subscriber sub = n.subscribe("exoskel_control", 100, arm_config);
	...
	while(ros::ok())
	{
		...
		if(flag==1)
		{
			if(arm_control.mode == 0)
			{
				motor_velocity = (long)(arm_control.velocity);
				cout << motor_velocity << endl;
				if((lResult = ModifiedProfileVelocityMode(g_pKeyHandle,g_usNodeId,ulErrorCode, motor_velocity))!=MMC_SUCCESS)
				{
					LogError("VelocityMode", lResult, ulErrorCode);
					return lResult;
				}                
			}
			else if(arm_control.mode ==1)
			{
				motor_position = (long)(arm_control.position);
				cout << motor_position << endl;
				if((lResult = ModifiedProfilePositionMode(g_pKeyHandle,g_usNodeId,ulErrorCode, motor_position))!=MMC_SUCCESS)
				{
			...
			{
				motor_current =(short)(arm_control.current);
				cout<< motor_current << endl;
                if((lResult= ModifiedProfileCurrentMode(g_pKeyHandle, g_usNodeId, &ulErrorCode,motor_current))!=MMC_SUCCESS)
				{
					return lResult;
				}
			} 
